    ...
    */
    ...
    if (match.score1 >= match.score2)
      this.userService.gainXP(match.user1,
        (match.user2.xpAmountForNextLevel * fixedPercentGain) / 100 +
        (match.user2.xpAmountForNextLevel * scorePercentGain) * (score_diff / max_score) / 100
      );
    else
      this.userService.gainXP(match.user1,
        (match.user2.xpAmountForNextLevel * fixedPercentLoss) / 100 +
        (match.user2.xpAmountForNextLevel * scorePercentLoss) * (score_diff / max_score) / 100
      );
    if (match.score2 >= match.score1)
      this.userService.gainXP(match.user2,
        (match.user1.xpAmountForNextLevel * fixedPercentGain) / 100 +
        (match.user1.xpAmountForNextLevel * scorePercentGain) * (score_diff / max_score) / 100
      );
    else
      this.userService.gainXP(match.user2,
        (match.user1.xpAmountForNextLevel * fixedPercentLoss) / 100 +
        (match.user1.xpAmountForNextLevel * scorePercentLoss) * (score_diff / max_score) / 100
      );
    this.userService.refreshRanks();
  }
