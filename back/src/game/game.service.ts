...
*/
  ...
  async CreateInvit(user1: User, user2: User) {
    if (await this.NotBlock(user1.ft_login, user2.ft_login)) {
      // deux user non block
      ...
      for (var [key, value] of this.s.entries()) {
        if (value) {
          if (value.ready === true) {
            ...
            if (!ft_login) ft_login = 'visitor';
            if (!ft_login2) ft_login2 = 'visitor';
  ...
  DisconnectionGame(client: Socket): Socket[] { // renvoit les deux clients
    ...
    for (var [key, value] of this.s.entries()) {
      if (value) {
        if (value.player1 == client || value.player2 == client) {
...
*/
  ...
  getScore(data: number) {
    ...
    if (room) return [room.score1, room.score2];
    return [0, 0];
  }
  ...
  sphere(room: Game): number[] {
    ...
    // if (room.xangle > 0.1) room.xangle -= 0.005;
    // if (room.xangle < -0.1) room.xangle += 0.005;
  ...
  async NotonlyBlock(user: User): Promise<[User, Socket]> {
    ...
    for (var [key, value] of iteratordispo) {
      if (value[0]) {
        ...
        if (
          (!blocked || blocked.status != UserRelationship.Status.BLOCKED) &&
          value[0].ft_login != user.ft_login
        )
          return value;
      }
    }
  ...
  Invitation(user: User): Invitation {
    for (var [key, value] of this.invitation.entries()) { // va passer sur toute les game
      if (value && (user === value.user1 || user === value.user2)) return value;
    }
    return null;
  }
  ...
  ClientChange(id_role: number[], client: Socket) {
    if (id_role) {
      ...
      if (id_role[1] === 1) this.s.get(id_role[0]).player1 = client;
      //room.player1 = client;
      if (id_role[1] === 2) this.s.get(id_role[0]).player2 = client;
      //room.player2 = client;
    }
  }
  ...
  async Idrole(client: Socket): Promise<number[]> {
    ...
    for (var [key, value] of this.s.entries()) { // va passer sur toute les game
      if (
        value &&
        (user.ft_login === value.user1.ft_login ||
          user.ft_login === value.user2.ft_login)
      ) {
        if (user.ft_login === value.user1.ft_login) return [key, 1];
        if (user.ft_login === value.user2.ft_login) return [key, 2];
      }
    }
    return null;
  }
  ...
  ReplaceMatched(user: User, client: Socket) {
    for (var [key, value] of this.dispoUser.entries()) {
      if (value && value[0]) {
        if (value[0] == user) value[1] = client;
      }
    }
    return null;
  }
  ...
  FindGame(user: User) {
    for (var [key, value] of this.s.entries()) {
      ...
      if (
        value.user1.ft_login == user.ft_login ||
        value.user2.ft_login == user.ft_login
      ) {
        return key;
      }
    }
  ...
  InsideDispo(user: User) {
    for (var [key, value] of this.dispoUser.entries()) {
      if (value && value[0] == user) return true;
    }
    return false;
  }
  ...
  async newGame(client: Socket): Promise<[number, boolean]> { //Promise<number[]>
    ...
    if (this.InsideGame(user) /*user.status == User.Status.PLAYING*/) {
      // si le user est deja en train de jouer ... Marche pas parce que meme player
      console.log('EST DEJA DANS UNE GAME');
      return [this.FindGame(user), true];
    }
    if (this.InsideDispo(user) /*user.status == User.Status.MATCHING*/) {
      // MARCHE PAS COMME CA
    ...
    if (contestant == null) {
      // Pas de Dispo Ou personne uniquement block
      ...
      return null;
    } // a un contestant creer le Match et le GAME EN MEME TEMPS
    else {
  ...
  getReady(id: number): boolean {
    if (this.s.get(id)) return this.s.get(id).ready;
    return false;
  }
  ...
  addtime(data: number) {
    var room = this.s.get(data);
    if (room) room.time = room.time + 1;
  }

  player2x_right(id: number, client: Socket) {
    if (this.s.get(id).player2 == client) this.s.get(id).Box2x += 2;
  }
  ...
  player1x_right(id: number, client: Socket) {
    if (this.s.get(id).player1 == client) this.s.get(id).Box1x += 2;
  }
  ...
  getRender(id: number): any {
    if (this.s.get(id)) return this.s.get(id).render;
    return null;
  }

  getTimer(id: number): any {
    if (this.s.get(id)) return this.s.get(id).timer;
    return null;
  }

  SetRender(id: number, render: any) {
    if (this.s.get(id)) return (this.s.get(id).render = render);
  }

  SetTimer(id: number, timer: any) {
    if (this.s.get(id)) return (this.s.get(id).timer = timer);
  }

  getBox1(id: number): number {
    if (this.s.get(id)) return Number(this.s.get(id).Box1x.toFixed(1));
    return 0;
  }
  ...
  endStream(client: Socket, id: number) {
    ...
    if (id != -1) {
      ...
      if (this.stream.get(id) === null) {
        // si le stream est null efface l index
  ...
  async endGame(
    client: Socket,
    id: number, // je ne sait pas si j implemente directement le score a la fin ou pendant
  ) {
  ...
  getClients(id: number): Socket[] {
    if (this.s.get(id)) return [this.s.get(id).player1, this.s.get(id).player2];
  ...
  getStream(data: number) {
    if (this.stream) return this.stream.get(data); // par default sera a 0
  }

  async CreateMatchID(
    data: number, // MET LES XP
  ) {
    console.log('MATCH history');
    var room = this.s.get(data);
    if (room) {
      const u1 = await User.findOne({
        where: { ft_login: room.player1.data.login },
      });
      const u2 = await User.findOne({
        where: { ft_login: room.player2.data.login },
      });
      if (u1 == null && u2 == null)
        // si les deux joueurs sont des visitor
        return;
      Match.update(data, { score1: room.score1, score2: room.score2 });
      if (u1) {
        if (room.score1 > room.score2) u1.gainXP(100);
        if (room.score1 < room.score2) u1.looseXP(50);
      }
      if (u2) {
        if (room.score2 > room.score1) u2.gainXP(100);
        if (room.score2 < room.score1) u2.looseXP(50);
      }
